...
"@
    if ((Test-Path -Path "D:\vdServerArchive\report_2_060.json" -PathType Leaf) -eq $false)
    {
        $json_report_file | ConvertTo-Json | Out-File "D:\vdServerArchive\report_2_060.json"
        # Get-Content C:\vdSA\servicename_report.json | ConvertFrom-Json
    }
    else 
    {
        Remove-Item -Path "D:\vdServerArchive\report_2_060.json"
        $json_report_file | ConvertTo-Json | Out-File "D:\vdServerArchive\report_2_060.json"
        $test = Get-Content "D:\vdServerArchive\report_2_060.json"
        Write-Host $test
    }
}
