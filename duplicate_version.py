            ...
            if check is False:
                for path_check_version in list_path_subfolder_achieve:
                    ...
                    if x:
                        ...
                        if float((datetime.date.today() - date_packup_file).days)* 0.002738 > 0:
                            backup_file_attribute_time.update({path_check_version:[factory, HDEV6, stage_production_SL, component ,path_check_version, date_packup_file]})
        path = list_path_subfolder_achieve
            
    
def create_report():
    with open("D:/vdSA/report.csv", 'w', encoding="UTF8") as f:
        header = ["Factory", "HDEV6","Stage/Product Line/SL", "Component", "Path", "Create Date"]
        writer = csv.writer(f)
...
def create_json_report():
    ...
    with open("D:/vdSA/report_duplicate_version.json", 'w') as f:
